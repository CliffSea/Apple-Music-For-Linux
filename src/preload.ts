...
import settingCSS from './settings/setting.css?raw'

webFrame.insertCSS(settingCSS);
...
window.addEventListener("DOMContentLoaded", async () => {
    ...
    <div id="wrapper-settings-overlay">
      <div id="wrapper-settings-modal">
        ...
        <div class="setting-row">
          ...
          <button id="lfm-login-btn" class="am-btn" style="display: ${settings.lastFmToken ? 'none' : 'block'};">1. Authorize in Browser</button>
          <button id="lfm-confirm-btn" class="am-btn success" style="display: none;">2. I have authorized!</button>
          <button id="lfm-logout-btn" class="am-btn" style="display: ${settings.lastFmToken ? 'block' : 'none'};">Log out</button> </div>
        <button id="save-settings-btn" class="am-btn secondary">Save & Close</button>
      </div>
    </div>
  `;
  ...
  const overlay = document.getElementById('wrapper-settings-overlay');
  const gearBtn = document.getElementById('wrapper-settings-btn');
  const saveBtn = document.getElementById('save-settings-btn');

  const lfmLoginBtn = document.getElementById('lfm-login-btn');
  const lfmLogoutBtn = document.getElementById('lfm-logout-btn');
  const lfmConfirmBtn = document.getElementById('lfm-confirm-btn');
  const lfmStatus = document.getElementById('lfm-status');

  gearBtn.addEventListener('click', () => overlay.style.display = 'flex');

  lfmLoginBtn.addEventListener('click', async () => {
    const success = await ipcRenderer.invoke('lastfm-login');
    if (success) {
      lfmLoginBtn.style.display = 'none';
      lfmConfirmBtn.style.display = 'block';
      lfmStatus.innerText = "Waiting for browser...";
      lfmStatus.style.color = "#ffcc00";
    }
  });

  lfmConfirmBtn.addEventListener('click', async () => {
    const username = await ipcRenderer.invoke('lastfm-complete-login');
    if (username) {
      lfmConfirmBtn.style.display = 'none';
      lfmLoginBtn.style.display = 'block';
      lfmLogoutBtn.style.display = 'block';
      lfmLoginBtn.innerText = "Re-authorize Account";
      lfmStatus.innerText = `Connected as ${username}`;
      lfmStatus.style.color = "#32d74b";
    } else {
      lfmStatus.innerText = `Login failed. Try again.`;
      lfmStatus.style.color = "#ff453a";
    }
  });
  ...
  lfmLogoutBtn.addEventListener('click', () => {
    const newSettings = {
      discordRpc: (document.getElementById('discord-toggle') as HTMLInputElement).checked,
      lastFmToken: ''
    };
